...
- (void)set$bottomByStretching:(CGFloat)bottom { self.frame = (CGRect){ .origin=self.frame.origin, .size.width=self.frame.size.width, .size.height=fmaxf(bottom-self.frame.origin.y,0) }; }
...
- (NSString *)frameDescription {
    return [NSString stringWithFormat:@"X: %f, Y: %f, W: %f, H: %f", self.$x, self.$y, self.$width, self.$height];
}
